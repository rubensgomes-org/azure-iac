# roots/09-postgresql/variables.tf
# -----------------------------------------------------------------------------
...
variable "location" {
  description = "Azure region for the PostgreSQL Flexible Server. Sourced from ../../envs/<env>/env.tfvars."
  type        = string
}

variable "apps" {
  description = "Microservice names. One database + one set of grants per name. Sourced from ../../envs/<env>/env.tfvars."
  type        = list(string)
  default     = []
}
...
variable "pg_entra_admin_group_name" {
  description = <<-EOT
    ...
    admin consent. Sourced from ../../envs/<env>/env.tfvars.
  EOT
  type        = string
...
variable "tags" {
  description = "Common tag map. Applied to the Flexible Server. Per-run override; committed defaults live in ../../envs/<env>/tags.json."
  type        = map(string)
  default     = {}
}
