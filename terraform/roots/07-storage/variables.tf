...
variable "location" {
  description = "Azure region for the storage account. Sourced from ../../envs/<env>/env.tfvars."
  type        = string
}

variable "apps" {
  description = "Microservice names. One blob container is created per name. Sourced from ../../envs/<env>/env.tfvars."
  type        = list(string)
  default     = []
}

variable "tags" {
  description = "Common tag map. Applied to the storage account. Per-run override; committed defaults live in ../../envs/<env>/tags.json."
  type        = map(string)
  default     = {}
}
