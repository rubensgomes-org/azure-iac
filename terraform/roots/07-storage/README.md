# 07-storage (terraform/roots)

Root Terraform config that provisions the shared Storage Account for the
target environment, one blob container per microservice, and the single RBAC
...
- `../../envs/<env>/env.tfvars` populated with `env`, `location` and `apps`.
...
az storage account list -g "rg-${TF_VAR_workload:-rgomes}data-${TF_VAR_env:-lab}" -o table
...
az storage container list \
  ...
  --auth-mode login \
  --query "[].name" -o tsv
# Expect one row per app in ../../envs/<env>/env.tfvars `apps`.
...
`st<workload>app<env>`, so reprovisioning the same env lands on the SAME name.
Azure keeps no soft-delete tombstone on a storage account name, so that name is
free again the moment the destroy completes — nothing to purge.
