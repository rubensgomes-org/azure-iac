# 10-container-app-environment (terraform/roots)

Root Terraform config that provisions the shared Azure Container Apps
Environment (ACAE / CAE) for the target environment. The environment hosts
...
```bash
cd terraform/roots/10-container-app-environment
...
az containerapp env show -g "rg-${TF_VAR_workload:-rgomes}app-${TF_VAR_env:-lab}" -n cae-${TF_VAR_workload:-rgomes}-${TF_VAR_env:-lab} \
...
delete it with `az containerapp delete -g "rg-${TF_VAR_workload:-rgomes}app-${TF_VAR_env:-lab}" -n <app> --yes`
before retrying.

Post-destroy, the CAE name (`cae-${TF_VAR_workload:-rgomes}-${TF_VAR_env:-lab}`) is immediately free — Container
...
- **Fixed name `cae-${TF_VAR_workload:-rgomes}-${TF_VAR_env:-lab}`** — no random suffix. Container App
  ...
  free of the naming dance PG / KV / LAW go through. `name` is
  ForceNew, so changing `workload`/`env` on a live environment is a
