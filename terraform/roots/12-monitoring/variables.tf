# roots/12-monitoring/variables.tf
# -----------------------------------------------------------------------------
# Root variables. Values come from `../../envs/<env>/env.tfvars` (shared,
# passed via -var-file). This module's own input (the action group email
# receiver) is a TF_VAR_ export -- this root is shared by every environment,
# so it can hold no `terraform.tfvars` of its own.
#
...
variable "location" {
  description = "Azure region for App Insights + action group. Sourced from ../../envs/<env>/env.tfvars."
  type        = string
}

variable "tags" {
  description = "Common tag map. Applied to App Insights + action group. Per-run override; committed defaults live in ../../envs/<env>/tags.json."
  type        = map(string)
  default     = {}
}
...
# ---- Consumed by this module (from a TF_VAR_ export) ------------------------
...
variable "action_group_email" {
  description = <<-EOT
    ...
    Supplied as TF_VAR_action_group_email — NOT in
    `../../envs/<env>/env.tfvars`, because the address is specific to this
    module's use case and other modules would ignore it. Not a
    `terraform.tfvars` here either: this root is shared by every environment,
    so an auto-loaded file would notify one address for all of them.
  EOT
  type        = string
}
