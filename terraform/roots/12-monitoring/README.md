...
- `TF_VAR_action_group_email` exported — the address the action group
  notifies. Required; this root has no default for it.
...
```bash
cd terraform/roots/12-monitoring
...
az monitor app-insights component show \
  -g "rg-${TF_VAR_workload:-rgomes}observability-${TF_VAR_env:-lab}" -a appi-${TF_VAR_workload:-rgomes}-${TF_VAR_env:-lab} \
  --query "{name:name, kind:kind, state:provisioningState, workspaceId:workspaceResourceId}" \
  -o table
# Expect kind=web, state=Succeeded, workspaceId ends with /workspaces/log-${TF_VAR_workload:-rgomes}-${TF_VAR_env:-lab}.

# Action group exists
az monitor action-group list -g "rg-${TF_VAR_workload:-rgomes}observability-${TF_VAR_env:-lab}" -o table
