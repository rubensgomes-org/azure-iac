# roots/04-managed-identities/variables.tf
# -----------------------------------------------------------------------------
...
variable "location" {
  description = "Azure region for the UAMI. Sourced from ../../envs/<env>/env.tfvars."
  type        = string
}

variable "tags" {
  description = "Common tag map. Applied to the UAMI. Per-run override; committed defaults live in ../../envs/<env>/tags.json."
  type        = map(string)
  default     = {}
}
