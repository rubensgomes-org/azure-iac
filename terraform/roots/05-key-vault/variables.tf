# roots/05-key-vault/variables.tf
# -----------------------------------------------------------------------------
...
variable "location" {
  description = "Azure region for the Key Vault. Sourced from ../../envs/<env>/env.tfvars."
  type        = string
}

variable "tags" {
  description = "Common tag map. Applied to the Key Vault. Per-run override; committed defaults live in ../../envs/<env>/tags.json."
  type        = map(string)
  default     = {}
}
