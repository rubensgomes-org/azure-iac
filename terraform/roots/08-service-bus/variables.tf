# roots/08-service-bus/variables.tf
# -----------------------------------------------------------------------------
# Root variables. Values come from `../../envs/<env>/env.tfvars` (shared,
# passed via -var-file) or, for this module's own inputs, a TF_VAR_ export.
#
...
variable "location" {
  description = "Azure region for the Service Bus namespace. Sourced from ../../envs/<env>/env.tfvars."
  type        = string
}

variable "tags" {
  description = "Common tag map. Applied to the Service Bus namespace. Per-run override; committed defaults live in ../../envs/<env>/tags.json."
  type        = map(string)
  default     = {}
}
...
variable "queues" {
  description = <<-EOT
    Queue names to create in the namespace. Supplied as TF_VAR_queues (not
    env.tfvars) — queue topology is a service-bus concern, not a shared-env
    concern. Empty list = namespace + RBAC only.
  EOT
  type        = list(string)
  default     = []
}
