...
# Registry name. Supplied as TF_VAR_acr_name, not from
# ../../envs/<env>/env.tfvars -- it is a per-module, per-environment value and
# env.tfvars is shared by all twelve roots. Not a `terraform.tfvars` here
...
variable "location" {
  description = "Azure region for the container registry. Sourced from ../../envs/<env>/env.tfvars."
  type        = string
}

variable "tags" {
  description = "Common tag map. Applied to the registry. Per-run override; committed defaults live in ../../envs/<env>/tags.json."
  type        = map(string)
  default     = {}
}
