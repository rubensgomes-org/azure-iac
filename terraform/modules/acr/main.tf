...
# constantly — it is baked into every image tag, every `docker push`, every
# `az acr` invocation, and every `apps_image_map` entry — and it is also the
# only name reachable from a workflow_dispatch input, so CI can point a run at
# an existing registry that a computed name could not address.
#
