...
Called by `terraform/roots/05-key-vault/`. State is owned by the caller —
...
| Type | Name | Notes |
|------|------|-------|
| `azurerm_key_vault` | `kv-<workload>-<env>` | SKU `standard`, RBAC mode, public network enabled. |
...
- **Public network enabled, `default_action = "Allow"`, bypass Azure
  ...
  `azurerm_private_endpoint` on `snet-<workload>pe-<env>`, wire it to
