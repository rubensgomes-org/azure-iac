...
# -----------------------------------------------------------------------------
# Name pattern: the plain CAF form psql-<workload>-<env> (e.g. "psql-rgomes-lab").
# Worst case 31 chars against a 63-char ceiling, so no precondition is needed.
#
# No random suffix: names are deterministic estate-wide. Two consequences worth
# knowing, both accepted in exchange for a name an operator can derive from
# env.tfvars. Server names are globally unique across every Azure tenant (they
# resolve as `<name>.postgres.database.azure.com`), so this can collide with a
# stranger's server. And Azure keeps a soft-delete tombstone on the name for up
# to 7 days after a drop, so a teardown/rebuild inside that window fails on the
# name — see docs/TEARDOWN.md.
#
...
resource "azurerm_postgresql_flexible_server" "this" {
  name                = "psql-${var.workload}-${var.env}"
