...
#
# Names follow the CAF form `<resource type>-<workload>-<environment>`. Where a
# resource is one of a per-key set (subnets, NSGs, VNet links) the key is folded
# onto the workload token rather than given a dash of its own — `snet-rgomesapp-lab`,
# not `snet-rgomes-app-lab` — so every name stays at three tokens. The private
# DNS zone names are the exception: those are dictated by Azure and cannot be
# chosen. See docs/NAMING.md.
# -----------------------------------------------------------------------------
...
resource "azurerm_virtual_network" "this" {
  name                = "vnet-${var.workload}-${var.env}"
...
resource "azurerm_subnet" "this" {
  ...
  name                 = "snet-${var.workload}${each.key}-${var.env}"
  resource_group_name  = var.resource_group_name
...
resource "azurerm_network_security_group" "this" {
  ...
  name                = "nsg-${var.workload}${each.key}-${var.env}"
  location            = var.location
  resource_group_name = var.resource_group_name
...
resource "azurerm_private_dns_zone_virtual_network_link" "this" {
  ...
  name = "vnet-link-${var.workload}${each.key}-${var.env}"
