# envs/dev/10-container-app-environment/variables.tf
# -----------------------------------------------------------------------------
...
variable "location" {
  description = "Azure region for the environment. Sourced from ../env.tfvars."
  type        = string
}

variable "tags" {
  description = "Common tag map. Applied to the environment. Per-run override; committed defaults live in ../tags.json."
  type        = map(string)
  default     = {}
}
