...
[INITIAL_SETUP.md](../../docs/INITIAL_SETUP.md) — never hardcode them here:
...
3. Ensure `bootstrap-backend/backend.tf` names the same three values you
   ...
   already carries the current values, and `envs/<env>/backend.hcl` must agree
   with it:
   ...
   terraform {
     backend "azurerm" {
       ...
       # root under terraform/roots/ uses its own `<module>/terraform.tfstate` key
       ...
       # project [INITIAL_SETUP.md](../../docs/INITIAL_SETUP.md) to access
       # the Blob Storage
...
5. To tear the backend down again, follow [TF_BOOTSTRAP_DESTROY.md](../../docs/TF_BOOTSTRAP_DESTROY.md).
...
[TF_BOOTSTRAP_DESTROY.md](../../docs/TF_BOOTSTRAP_DESTROY.md); in outline it is:
