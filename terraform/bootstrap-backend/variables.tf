...
# `terraform/envs/<env>/env.tfvars`, so a bare `terraform apply` with no tfvars
...
#
# The `-lab` token is a naming concession, not a scoping one: this RG holds the
# state for EVERY environment (see the note on `var.tags` below), and the
# environments are separated one level down, by container. A `dev` estate's
# state blobs live in a container inside this lab-named group.
#
...
#
# Convention: `<workload>-<env>-tfstate`. This is the one backend coordinate
# that is genuinely per-environment, so it is the one whose env token means
# something -- `rgomes-lab-tfstate`, `rgomes-dev-tfstate`.
#
# MUST match `container_name` in backend.tf.
variable "container_name" {
  type        = string
  description = "Blob container name for Terraform state."
  default     = "rgomes-lab-tfstate"
}
...
variable "owner" {
  ...
  default     = "rubens.gomes@3cloudsolutions.com"
}
...
# `terraform/envs/<env>/env.tfvars`). The `owner` tag is NOT set here — it comes
...
# spans every environment. That the RG and Storage Account NAMES now carry a
# `-lab` / `02` token does not change what they hold: every environment's state
# lives here, separated by container. An `environment` tag would assert
# something about the contents that is not true.
#
