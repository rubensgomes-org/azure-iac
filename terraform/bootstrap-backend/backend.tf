...
#   # (gitignored) and with `envs/<env>/backend.hcl`.
#   backend "azurerm" {
#     # -------------------------------------------------------------------------
...
#     # see docs/INITIAL_SETUP.md), update the literals here to match.
#
...
#     # Every module root under terraform/roots/ supplies its own key at init time via
