...
The values below are for the `lab` environment, which is the default. `dev`
exists as a config directory too — see "Selecting an environment" after this
section for what has to change to target it.
...
- Configure the following Terraform (tfvars) shell environment variables:
    ...
    # CAF workload token. With TF_VAR_env it names EVERY resource in the estate
    # as <type>-<workload>-<env>: rg-rgomesapp-lab, kv-rgomes-lab,
    # strgomesapplab, and so on.
    export TF_VAR_workload='rgomes'
    # export TF_VAR_action_group_email='rubens.s.gomes@gmail.com'
    export TF_VAR_action_group_email='rubens.gomes@3cloudsolutions.com'
    export TF_VAR_pg_entra_admin_group_name="az-${ENV}-pg-admins"
    ...
    #export TF_VAR_acr_name="crrgomes${ENV}01"
    export TF_VAR_acr_name="crrgomes${ENV}02"
    #export TF_VAR_owner='rubens.s.gomes@gmail.com'
    export TF_VAR_owner='rubens.gomes@3cloudsolutions.com'
    ...
    export TF_VAR_backend_resource_group_name="rg-rgomestfstate-${ENV}"
    export TF_VAR_container_name="rgomes-${ENV}-tfstate"
    # ATTENTION: this MUST be a globally unique ID in Azure
    # export TF_VAR_storage_account_id='strgomestfstate01'
    export TF_VAR_storage_account_id='strgomestfstate02'
    ```
...
`lab` is the default: `ENV` defaults to it in the Makefile and the exports
above are lab's. Targeting another environment means changing `ENV` and the
exports that must move with it:

```bash
# picks envs/dev/backend.hcl
make ENV=lab ...
# names resources, picks envs/dev/tags.json
export TF_VAR_env="${ENV}"
# lab's own state container
export TF_VAR_container_name="rgomes-${ENV}-tfstate"
# registry names are globally unique
export TF_VAR_acr_name='<globally unique>'
```

`make check-backend` runs ahead of every `init`, `apply`, `destroy` and
`pull-state`, and fails if `ENV`, `TF_VAR_env` and `TF_VAR_container_name`
disagree. That matters because the environment variable OUTRANKS `backend.hcl`
at init time, so an export left over from another environment's session would
otherwise point this run at that environment's state without saying so.
...
### Provision Terraform Bootstrap Backend

The next step is to provision the Terraform Bootstrap Backend resources (e.g.
, resource group, storage account, and blob container) which are used to
store the Terraform state information in Azure.

Follow [TF_BOOTSTRAP_CREATE](TF_BOOTSTRAP_CREATE.md).

## GitHub Actions Secrets and Variables

- Project Repository Action Variables below:
