...
1. All the Azure resources (`terraform/roots/` modules) provisioned and
   maintained by this Terraform project must be properly removed prior to
   destroying the Terraform bootstrap backend resources below.
...
[main.tf](../terraform/bootstrap-backend/main.tf) are deleted.
...
2. Check all the `terraform/modules` resource groups are gone
   ...
   # check the following list of resource groups to ensure you do NOT have 
   # any resource group pending to be removed.
   az group list --query "[?starts_with(name,'rg-')].{Name:name,Location:location,State:properties.provisioningState}"\
   -o table
   ```

**NOTE: If any of your Azure estate resource groups are listed above, stop and
clean up those resources before continuing.**
...
1. Restore the [backend.tf](../terraform/bootstrap-backend/backend.tf) so the committed tree matches what a
