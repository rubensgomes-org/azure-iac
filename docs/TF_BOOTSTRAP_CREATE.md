...
[main.tf](../terraform/bootstrap-backend/main.tf) are created.
...
NOTE: if you see any resource groups listed above, STOP because you already have
the Terraform state backend resources already created.
...
Open [backend.tf](../terraform/bootstrap-backend/backend.tf) and comment out the
entire `terraform { backend "azurerm" { ... } }` block.
...
- Do not migrate the state if the previous state is no longer needed. For
  example, if you are resetting the Terraform state to its initial state or have
  completely destroyed your Azure environment.
...
- Migrate the Terraform state from the local `terraform.tfstate` file to the
  ...
  # answer no if you do NOT need to migrate the previous state.
  ```
...
#### Note - In case your local state is not synced with backend:

```bash
RG="/subscriptions/${AZURE_SUBSCRIPTION_ID}/resourceGroups/${TF_VAR_backend_resource_group_name}"
SA="$RG/providers/Microsoft.Storage/storageAccounts/${TF_VAR_storage_account_id}"

terraform import azurerm_resource_group.tfstate "$RG"
terraform import azurerm_storage_account.tfstate "$SA"
terraform import azurerm_storage_container.tfstate "$SA/blobServices/default/containers/rgomes-lab-tfstate"
terraform import azurerm_role_assignment.state_blob_contributor \
  "$SA/providers/Microsoft.Authorization/roleAssignments/9a0f5f99-ebae-f806-2a28-15f618015e95"
terraform import 'azurerm_management_lock.tfstate_rg_lock[0]' \
  "$RG/providers/Microsoft.Authorization/locks/tfstate-can-not-delete"
```
...
`terraform/roots/` can now `init` against this backend.
